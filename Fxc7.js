...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Anas Bex\n'
            + 'ORG:Owner Naesan;\n'
            + 'TEL;type=CELL;type=VOICE;waid=62895618516524:+62 895-6185-16524\n'
            + 'END:VCARD'
...
async function starts() {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const AnasBexGans = ["0@s.whatsapp.net"]
			const AnasBexGans2 = "~ a n a s  b e x ~"
			const isUrl = (url) => {
			    return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		}
			const reply = (teks) => {
				anasbex.sendMessage(from, teks, text, {quoted:mek})
		}
...
if (isGroup) {
		...
		if (ANS.includes("://chat.whatsapp.com/")){
		if (!isGroup) return
